...
mappings {
    if(!parent) {
        ...
        path("/receiveEventData") {action: [GET: "receiveEventData"]}
    }
}
...
def receiveEventData(data) {
    log.debug "receiveEventData: $data"
}

def startStreamTest() {
    def ip = "10.0.0.134"
    def port = 3000
    def apiUrl = apiServerUrl("/api/token/${accessToken}/smartapps/installations/${app.id}/receiveEventData")
